...
static void cluster_dump_nodes(redisCluster *c) {
    ...
    ZEND_HASH_FOREACH_PTR(c->nodes, p) {
        ...
        php_printf("%d %s %d %d", p->sock->port, slave,p->sock->prefix_len, p->slot);
        php_printf("\n");
    ...
    while (elements-- > 0) {
        ...
        switch (r->type) {
            ...
            case TYPE_MULTIBULK:
                ...
                r->elements = r->len;
                cluster_multibulk_resp_recursive(sock, r->elements, r->element, err);
                if (*err) {
					return;
				}
                break;
                                    ...
                                    ulong slaveidx)
{
    ...
    if (!SLOT_SLAVES(c, slot) || (node = zend_hash_index_find_ptr(SLOT_SLAVES(c,slot), slaveidx)) == NULL) {
        return NULL;
    }
...
clusterReply*
cluster_read_sock_resp(RedisSock *redis_sock, REDIS_REPLY_TYPE type, size_t len)
{
    clusterReply *r;
    ...
    switch (r->type) {
        ...
        case TYPE_MULTIBULK:
            ...
            if (len != (size_t)-1) {
                ...
                cluster_multibulk_resp_recursive(redis_sock, len, r->element, &err);
            }
            break;
        default:
            cluster_free_reply(r,1);
...
static int cluster_send_asking(RedisSock *redis_sock) {
    return cluster_send_direct(redis_sock, RESP_ASKING_CMD, sizeof(RESP_ASKING_CMD)-1, TYPE_LINE);
}
...
static int cluster_send_readonly(RedisSock *redis_sock) {
    ...
    if (redis_sock->readonly) {
		return 0;
	}

    /* Return success if we can send it */
    ret = cluster_send_direct(redis_sock, RESP_READONLY_CMD, sizeof(RESP_READONLY_CMD)-1, TYPE_LINE);
...
static int cluster_send_multi(redisCluster *c, short slot) {
    if (cluster_send_direct(SLOT_SOCK(c,slot), RESP_MULTI_CMD, sizeof(RESP_MULTI_CMD)-1, TYPE_LINE) == 0) {
...
PHP_REDIS_API int cluster_send_exec(redisCluster *c, short slot) {
    ...
    /* Send exec */
    retval = cluster_send_slot(c, slot, RESP_EXEC_CMD, sizeof(RESP_EXEC_CMD)-1, TYPE_MULTIBULK);
...
PHP_REDIS_API int cluster_send_discard(redisCluster *c, short slot) {
    if (cluster_send_direct(SLOT_SOCK(c,slot), RESP_DISCARD_CMD, sizeof(RESP_DISCARD_CMD)-1, TYPE_LINE)) {
        return 0;
    }
    return -1;
}
...
static void cluster_dist_free_ht(zval *p) {
    ...
    for (i = 0; i < dl->len; i++) {
		if (dl->entry[i].key) {
			zend_string_release(dl->entry[i].key);
		} 
		if (dl->entry[i].val) {
			zend_string_release(dl->entry[i].val);
		}
    }
...
static clusterKeyVal *cluster_dl_add_key(clusterDistList *dl, zend_string *key)
{
    ...
    // Set key info
    dl->entry[dl->len].key = zend_string_copy(key);
...
int cluster_dist_add_key(redisCluster *c, HashTable *ht, zend_string *key, clusterKeyVal **kv)
{
    ...
    retptr = cluster_dl_add_key(dl, key);
...
void cluster_dist_add_val(redisCluster *c, clusterKeyVal *kv, zval *z_val)
{
    ...
    // Serialize our value
    val = redis_serialize(c->flags, z_val);
...
void cluster_multi_free(clusterMultiCmd *mc) {
	smart_str_free(&mc->cmd);
	smart_str_free(&mc->args);
}
...
void cluster_multi_fini(clusterMultiCmd *mc) {
    ...
    smart_str_appendl(&(mc->cmd), ZSTR_VAL(mc->args.s), ZSTR_LEN(mc->args.s));
}
                    ...
                    unsigned short port, unsigned short slot, short slave)
{
    ...
    // Attach socket
    node->sock = redis_sock_create(host, host_len, port, c->timeout, 0, NULL, 0, 1);
...
#define VALIDATE_SLOTS_OUTER(r) \
    (r->elements >= 3 && r2->element[0]->type == TYPE_INT && r->element[1]->type == TYPE_INT)
...
static redisClusterNode *cluster_get_asking_node(redisCluster *c) {
    ...
    pNode = cluster_node_create(c, c->redir_host, c->redir_host_len, c->redir_port, c->redir_slot, 0);
...
cluster_init_seeds(redisCluster *cluster, HashTable *ht_seeds) {
    ...
    ZEND_HASH_FOREACH_VAL(ht_seeds, z_seed) {
        ...
        redis_sock = redis_sock_create(str,
				psep-str, (unsigned short)atoi(psep + 1), cluster->timeout, 0, NULL, 0, 0);

        // Index this seed by host/port
        key_len = snprintf(key, sizeof(key), "%s:%u", redis_sock->host, redis_sock->port);
...
static int cluster_check_response(redisCluster *c, REDIS_REPLY_TYPE *reply_type)
{
    ...
    if (redis_sock_gets(c->cmd_sock, c->line_reply, sizeof(c->line_reply), &sz) < 0) {
        return -1;
    }
                                           ...
                                           unsigned short port)
{
    int key_len;
    char key[1024];
    redisClusterNode *ret;

    key_len = snprintf(key, sizeof(key), "%s:%d", host, port);

    if ((ret = zend_hash_str_find_ptr(c->nodes, key, key_len)) != NULL) {
        return ret;
    }
                             ...
                             int cmd_len, REDIS_REPLY_TYPE rtype)
{
    ...
    if (c->flags->mode == MULTI && c->cmd_sock->mode != MULTI) {
        if (cluster_send_multi(c, slot) == -1) {
            zend_throw_exception(redis_cluster_exception_ce, "Unable to enter MULTI mode on requested slot", 0);
            return -1;
        }
    }
                                  ...
                                  int cmd_len)
{
    ...
    do {
        ...
        if (cluster_sock_write(c, cmd, cmd_len, 0) == -1) {
            ...
            zend_throw_exception(redis_cluster_exception_ce, "Can't communicate with any node in the cluster", 0);
            return -1;
        }
        ...
        if (resp == 1) {
           ...
           if (c->flags->mode == MULTI) {
               zend_throw_exception(redis_cluster_exception_ce,
                   "Can't process MULTI sequence when cluster is resharding", 0);
               return -1;
           }
    ...
    if (c->clusterdown) {
        zend_throw_exception(redis_cluster_exception_ce, "The Redis Cluster is down (CLUSTERDOWN)", 0);
        return -1;
    } else if (timedout) {
        zend_throw_exception(redis_cluster_exception_ce, "Timed out attempting to find data in the correct node!", 0);
    }
...
PHP_REDIS_API void cluster_bool_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
...
PHP_REDIS_API void cluster_ping_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
...
PHP_REDIS_API void cluster_1_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
...
PHP_REDIS_API void cluster_long_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
...
PHP_REDIS_API void cluster_type_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
...
PHP_REDIS_API void cluster_sub_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
    subscribeContext *sctx = (subscribeContext*)ctx;
    zval *z_tab, *z_tmp, ret, rv, z_args[4];
    int pull = 0;
	int i;

    // Consume each MULTI BULK response (one per channel/pattern)
    while (sctx->argc--) {
        z_tab = cluster_zval_mbulk_resp(INTERNAL_FUNCTION_PARAM_PASSTHRU, c, pull, mbulk_resp_loop_raw, &rv);
    ...
    while (1) {
        ...
        z_tab = cluster_zval_mbulk_resp(INTERNAL_FUNCTION_PARAM_PASSTHRU, c, 1, mbulk_resp_loop, &rv);
        ...
        if (zend_call_function(&(sctx->cb), &(sctx->cb_cache)) != SUCCESS) {
            break;
        }
    ...
    if (CLUSTER_IS_ATOMIC(c)) {
        switch (r->type) {
            ...
            case TYPE_MULTIBULK:
                ...
                for (i = 0; i < r->elements; i++) {
                    cluster_mbulk_variant_resp(r->element[i], return_value);
                }
                              ...
                              REDIS_SCAN_TYPE type, long *it)
{
    ...
    // Read the BULK size
    if (cluster_check_response(c, &c->reply_type) != 0 || c->reply_type != TYPE_BULK) {
        return FAILURE;
    }
                              ...
                              void *ctx)
{
    ...
    z_result = redis_parse_info_response(info, &rv);
    efree(info);
                                     ...
                                     void *ctx)
{
    ...
    z_result = redis_parse_client_list_response(info, &rv);
    efree(info);
                                     ...
                                     redisCluster *c, int pull, mbulk_cb cb, zval *rv)
{
    ...
    // Call our callback
    if (cb(c->cmd_sock, rv, c->reply_len, NULL) == FAILURE) {
        zval_dtor(rv);
        return NULL;
    }
...
PHP_REDIS_API void cluster_mbulk_mget_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
	short fail;
    ...
    if (mctx->last) {
        if (CLUSTER_IS_ATOMIC(c)) {
			ZVAL_COPY_VALUE(return_value, &mctx->z_multi);
        } else {
...
/* Handler for MSETNX */
PHP_REDIS_API void cluster_msetnx_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
    clusterMultiCtx *mctx = (clusterMultiCtx*)ctx;
    ...
    if (c->reply_type != TYPE_INT) {
        php_error_docref(0, E_WARNING, "Invalid response type for MSETNX");
        while (real_argc--) {
            add_next_index_bool(&mctx->z_multi, 0);
        }
        return;
...
/* Handler for DEL */
PHP_REDIS_API void cluster_del_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
    clusterMultiCtx *mctx = (clusterMultiCtx*)ctx;

    // If we get an invalid reply, inform the client
    if (c->reply_type != TYPE_INT) {
        php_error_docref(0, E_WARNING, "Invalid reply type returned for DEL command");
        efree(mctx);
        return;
    }

    // Increment by the number of keys deleted
    Z_LVAL(mctx->z_multi) += c->reply_len;

    if (mctx->last) {
        if (CLUSTER_IS_ATOMIC(c)) {
            ZVAL_LONG(return_value, Z_LVAL(mctx->z_multi));
        } else {
            add_next_index_long(&c->multi_resp, Z_LVAL(mctx->z_multi));
        }
		ZVAL_UNDEF(&mctx->z_multi);
    }
                              ...
                              void *ctx)
{
    ...
    if (mctx->last) {
        if (CLUSTER_IS_ATOMIC(c)) {
            ZVAL_BOOL(return_value, Z_TYPE(mctx->z_multi) == IS_TRUE ? 1 : 0);
        } else {
            add_next_index_bool(&c->multi_resp, Z_TYPE(mctx->z_multi) == IS_TRUE ? 1 : 0);
        }
    }
...
PHP_REDIS_API void
cluster_mbulk_zipdbl_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
    cluster_gen_mbulk_resp(INTERNAL_FUNCTION_PARAM_PASSTHRU, c, mbulk_resp_loop_zipdbl, NULL);
}

/* Associate multi bulk response (for HMGET really) */
PHP_REDIS_API void
cluster_mbulk_assoc_resp(INTERNAL_FUNCTION_PARAMETERS, redisCluster *c, void *ctx)
{
    cluster_gen_mbulk_resp(INTERNAL_FUNCTION_PARAM_PASSTHRU, c, mbulk_resp_loop_assoc, ctx);
}
                          ...
                          long long count, void *ctx)
{
    ...
    while (count--) {
        ...
        if (line != NULL) {
            ...
            if (redis_unserialize(redis_sock, line, line_len, &z) == 1) {
                efree(line);
                add_assoc_zval_ex(z_result, Z_STRVAL(z_keys[i]), Z_STRLEN(z_keys[i]), &z);
            } else {
                add_assoc_stringl_ex(z_result, Z_STRVAL(z_keys[i]), Z_STRLEN(z_keys[i]), line, line_len);
            }
        } else {
